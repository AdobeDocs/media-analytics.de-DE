...
| --- | --- | --- |
| Inhalt | eVar <br>Standardgültigkeit: Besuch | Festlegung eines eigenen eVars. |
| Content-Typ | eVar<br> Standardgültigkeit: Seitenansicht | Festlegung eines eigenen eVars. |
...
| mediaPlayerName | `mediaPlayerName` (erforderlich): Der Name des Medienplayers, mit dem das Video wiedergegeben wird, wie er in Videoberichten angezeigt werden soll. | Setzen von eVar- oder Kontextdatenvariablen im Link-Aufruf. | `s.contextData['video.player']` <br> `  = ”CustomPlayer Name”;` |
...
| playerName | `playerName`: (erforderlich) Name des Medienplayers, mit dem die Anzeige wiedergegeben wird. | nicht angegeben | Nicht verfügbar. |
...
| Media.click | `s.Media.click(name, offset)` | `s.tl()` | Verwenden Sie einen benutzerdefinierten Link-Analyseaufruf, um Klicks zu verfolgen. |
