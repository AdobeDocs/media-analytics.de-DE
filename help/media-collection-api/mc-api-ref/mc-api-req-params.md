...
| Anforderungs-Schlüssel  | Erforderlich | Festgelegt in |  Beschreibung  |
| --- | :---: | :---: | --- |
...
| `analytics.visitorId` | N | `sessionStart` | Die Adobe-Besucher-ID ist eine benutzerdefinierte ID, die Sie für mehrere Adobe-Anwendungen verwenden können. The Heartbeat `visitorId` equals the Analytics `VID.` |
...
| `visitor.marketingCloudUserId` | N | `sessionStart` | Dies ist die Experience Cloud-Benutzer-ID (ECID). In den meisten Fällen ist dies die ID, mit der Sie einen Benutzer identifizieren sollten. The Heartbeat `marketingCloudUserId` equals the `MID` in Adobe Analytics. Dieser Parameter ist zwar technisch nicht erforderlich, ist jedoch für den Zugriff auf die Experience Cloud-Familie von Apps erforderlich. |
...
| `media.resume` | N | `sessionStart` | Gibt an, ob ein Benutzer eine vorherige Sitzung wiederaufnimmt (anders als beim Starten einer neuen Sitzung) |
...
>[!NOTE]
>
>Media Analytics (MA) ist in die Experience Cloud-Familie von Apps integriert (Adobe Analytics, Audience Manager, Target usw.). Sie benötigen eine Experience Cloud ID, um auf diese Anwendungen zuzugreifen. _Die ECID dient der Identifizierung der Benutzer in den meisten Szenarien._

### appInstallationId

* **Wenn Sie *keinen*`appInstallationId`Wert übergeben -** Das MA-Back-End generiert keine MCID mehr, sondern stattdessen auf Adobe Analytics. Adobe empfiehlt, entweder eine MCID (sofern verfügbar) oder eine `appInstallationId` zu senden (neben der erforderlichen `marketingCloudOrgId`), damit die Mediensammlungs-API die MCID generiert und bei allen Aufrufen sendet.

* **Wenn *Sie Werte*übergeben`appInstallationId`-** Die MCID *kann durch das MA-Back-End* generiert werden, wenn Sie Werte für `appInstallationId` und die (erforderlichen) `marketingCloudOrgId` Parameter übergeben. Wenn Sie `appInstallationId` nicht selbst übergeben, muss der Wert clientseitig persistent sein. Er muss außerdem eindeutig für die Anwendung auf dem Gerät sein und beibehalten werden, bis die Anwendung neu installiert wird.
...
* **analytics. aid:**
   ...
   Der Wert dieses Schlüssels muss eine Zeichenfolge sein, die die Analytics-Legacy-Benutzer-ID darstellt.
* **visitor. customerids:**
...
Dieser Parameter gibt an, welche Adobe Audience Manager (AAM) Edge erreicht, wenn Adobe Analytics die Kundendaten an Audience Manager sendet. Wenn Sie diesen Parameter nicht übergeben, wird er von Adobe fest auf 1 codiert. Das ist insbesondere dann wichtig, wenn Endanwender ihre Geräte an geografisch weit entfernten Standorten verwenden (z. B. US-Ost- und -Westküste, Europa, Asien). Andernfalls werden die Daten auf verschiedene AAM-Edges aufgeteilt.
